...
// Endpoint AJAX para actualizar mesero
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'update_mesero') {
    header('Content-Type: application/json');
    ...
    // ✅ CORREGIDO: Leer el valor real del campo estado (1 si es '1', 0 en cualquier otro caso)
    ...
    if (empty($errores['correo'])) {
        $check = $conexion->prepare("SELECT id_mesero FROM meseros WHERE correo = ? AND id_mesero != ?");
        $check->bind_param("si", $correo, $id);
        $check->execute();
        $check->store_result();
        if ($check->num_rows > 0) {
            $errores['correo'] = "Este correo ya está registrado por otro mesero";
        }
        $check->close();
    }
    ...
    if (!empty($password)) {
        ...
        $sql = "UPDATE meseros SET nombre=?, correo=?, telefono=?, fecha_nacimiento=?, password=?, estado=? WHERE id_mesero=?";
        $stmt = $conexion->prepare($sql);
        $stmt->bind_param("sssssii", $nombre, $correo, $telefono, $fecha_nacimiento, $hash, $estado, $id);
    } else {
        $sql = "UPDATE meseros SET nombre=?, correo=?, telefono=?, fecha_nacimiento=?, estado=? WHERE id_mesero=?";
        $stmt = $conexion->prepare($sql);
        $stmt->bind_param("ssssii", $nombre, $correo, $telefono, $fecha_nacimiento, $estado, $id);
    }
    
    if ($stmt->execute()) {
        echo json_encode(['success' => true, 'message' => 'Mesero actualizado correctamente']);
    } else {
...
<div id="editModal" class="modal">
    <div class="modal-content edit-modal-content">
        ...
        <form id="editForm" novalidate>
            ...
            <!-- Checkbox con name="estado" y value="1" -->
...
function openEditModal(meseroData) {
    document.getElementById('edit_id').value = meseroData.id;
    document.getElementById('edit_nombre').value = meseroData.nombre;
    document.getElementById('edit_correo').value = meseroData.correo;
    document.getElementById('edit_telefono').value = meseroData.telefono || '';
    document.getElementById('edit_fecha').value = meseroData.fecha || '';
    document.getElementById('edit_estado').checked = (meseroData.estado == 1);
...
document.getElementById('editForm').addEventListener('submit', async function(e) {
    ...
    formData.append('ajax_action', 'update_mesero');
    
    // Forzar el valor correcto del estado
...
document.addEventListener('click', function(e) {
    if (e.target.closest('.edit-btn')) {
        ...
        openEditModal(meseroData);
    }
});

// Actualizar tabla sin caché y con edad corregida
async function refreshTable() {
    try {
        ...
        tbody.innerHTML = meseros.map(m => {
            ...
            if (m.fecha_nacimiento) {
                const hoy = new Date();
                const nac = new Date(m.fecha_nacimiento + 'T00:00:00');
                ...
                <tr data-id="${m.id_mesero}">
                    ...
                    <td>
                        <button class="action-btn edit-btn" 
                                ...
                                data-correo="${escapeHtml(m.correo)}"
                                data-telefono="${m.telefono ? escapeHtml(m.telefono) : ''}"
                                data-fecha="${m.fecha_nacimiento || ''}"
                                data-estado="${m.estado}">
                            <i class="fas fa-edit"></i> Editar
                        </button>
...
// Validaciones del formulario de registro (se mantienen igual)
